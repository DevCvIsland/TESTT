...
import { getPankoninexRabbitContract } from '../utils/contracts'
...
const NftProvider: React.FC<NftProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const fetchContractData = async () => {
      try {
        const koninexRabbitsContract = getPankoninexRabbitContract()
        ...
        const balanceOf = await koninexRabbitsContract.methods.balanceOf(account).call()
        ...
        if (balanceOf > 0) {
          const getTokenIdAndBunnyId = async (index: number) => {
            try {
              const tokenId = await koninexRabbitsContract.methods.tokenOfOwnerByIndex(account, index).call()
              const bunnyId = await koninexRabbitsContract.methods.getBunnyId(tokenId).call()
