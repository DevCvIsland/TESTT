...
import { Button, Modal, Text } from '@koninex/uikit'
...
const ClaimNftModal: React.FC<ClaimNftModalProps> = ({ nft, onSuccess, onDismiss }) => {
  ...
  const koninexBalance = useTokenBalance(getKoninexAddress())
  const koninexInWallet = getBalanceNumber(koninexBalance)
  ...
  useEffect(() => {
    if (koninexInWallet === 0) {
      setError('You must have a KNX balance greater than zero to claim NFT')
    }
  }, [koninexInWallet, setError])
  ...
  return (
    <Modal title={TranslateString(999, 'Claim NFT')} onDismiss={onDismiss}>
      ...
      <Actions>
        ...
        <Button fullWidth onClick={handleConfirm} disabled={!account || isLoading || koninexInWallet <= 0}>
